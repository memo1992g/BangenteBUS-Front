...
export function ParametersConfig() {
  ...
  const load = async () => {
    try {
      setError("");
      const res = await SystemParamsService.list();
  ...
  const run = async (action: "create" | "get" | "update" | "delete") => {
    try {
      ...
      if (action === "create") await SystemParamsService.create(JSON.parse(payload || "{}"));
      if (action === "get") setItems([await SystemParamsService.getById(id)]);
      if (action === "update") await SystemParamsService.update(id, JSON.parse(payload || "{}"));
      if (action === "delete") await SystemParamsService.remove(id);
      await load();
