...
export function MessagesConfig() {
  ...
  const run = async (action: "create" | "get" | "update" | "delete") => {
    try {
      ...
      if (action === "create") await MessagesService.create(JSON.parse(payload || "{}"));
      if (action === "get") setItems([await MessagesService.getById(id)]);
      if (action === "update") await MessagesService.update(id, JSON.parse(payload || "{}"));
      if (action === "delete") await MessagesService.remove(id);
      await load();
